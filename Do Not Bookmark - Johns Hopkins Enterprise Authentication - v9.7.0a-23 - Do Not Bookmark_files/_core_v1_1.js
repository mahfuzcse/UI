...
function fingerprint_useragent() {
    ...
    try {
        ...
        if (glbOutputMode === "RSA") {
			/* appVersion and platform are supported by all major browsers */
            strTmp = strUserAgent + glbSep + navigator.appVersion + glbSep + navigator.platform;
            if (strIE) {
               	/* appMinorVersion only supported in IE and Opera */
               	try {
               		strTmp += glbSep + navigator.appMinorVersion;
				} catch (e) {
					strTmp += glbSep + 'na';
				}
				/* cpuClass only supported in IE */
            	try {
            		strTmp += glbSep + navigator.cpuClass;
				} catch (e) {
                	strTmp += glbSep + 'na';
				}
                /* browserLanguage only supported in IE, Safari and Chrome */
                try {
                	strTmp += glbSep + navigator.browserLanguage;
				} catch (e) {
					strTmp += glbSep + 'na'
				}
				/* ScriptEngineBuildVerstion supported in IE */
                try {
                ...
                /* language is supported by all major browsers */
                strTmp += glbSep + navigator.language;
            }
        } else {
			/* platform are supported by all major browsers */
...
function fingerprint_os() {
    ...
    try {
		...
		if (strUserAgent.indexOf("windows nt 8") !== -1) {
            ...
            strOS = "Windows 10";
	} else if (strUserAgent.indexOf("windows nt 6.4") !== -1) {
            strOS = "Windows 10";
            ...
            strOS = "Windows 95";
        } else if (strUserAgent.indexOf("windows ce") !== -1) {
            strOS = "Windows CE";
        ...
        } else if ((strUserAgent.indexOf("mac os x 10.1") !== -1) && (strUserAgent.indexOf("mac os x 10.12") == -1)) {
            strOS = "Mac OSX Puma";
		...
		} else if ((strUserAgent.indexOf("mac os x 10_12") !== -1) || (strUserAgent.indexOf("mac os x 10.12") !== -1)) {
				strOS = "Mac OS Sierra";
...
function fingerprint_browser() {
    ...
    try {
        ...
        if (/msie (\d+\.\d+);/.test(strUserAgent)) { //test for MSIE x.x;
            ...
            numVersion = 11;
            strBrowser = "Internet Explorer " + numVersion;
	} else if (strUserAgent.indexOf("edge/12") > -1) {
            numVersion = 12;
            strBrowser = "Edge 12";
...
function fingerprint_plugins() {
    ...
    try {
        ...
        if (glbOutputMode === "RSA") {
            ...
            htMozComponents.put('nprpjplug.dll', 'rpv'); // Real Player Version
           // htMozComponents.put('NPSWF32.dll', 'swf'); // Adobe Flash
           // htMozComponents.put('NPSWF32_11_7_700_202.dll', 'swf11'); // Adobe Flash 11*
	    htMozComponents.put('npdsplay.dll', 'wpm'); // Windows Media Player
        }
